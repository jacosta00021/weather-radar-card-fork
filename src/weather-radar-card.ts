...
export class WeatherRadarCard extends LitElement implements LovelaceCard {
  ...
  protected firstUpdated(): void {
    ...
    if (this.isPanel) {
      const height = this.offsetParent 
        ? this.offsetParent.clientHeight - 48 - 2 - (this.editMode === true ? 59 : 0)
        : 540;
      this.style.setProperty('--panel-radar-height', `${height}px`);
    }

    if (this._config.square_map) {
      this.style.setProperty('--square-map-height', `${this.getBoundingClientRect().width}px`);
    }
  }
  ...
  protected render(): TemplateResult | void {
    ...
    const mapClass = this.isPanel ? 'radar-map panel-mode' : 
                    this._config.square_map ? 'radar-map square-map' : 
                    'radar-map';
    ...
    const doc = `
      ...
      <html>
        ...
        <body>
          ...
          <div id="div-progress-bar" class="progress-bar-container">
            <div id="progress-bar" class="progress-bar"></div>
          </div>
          <div id="bottom-container" class="bottom-container">
            <div id="timestampid" class="timestamp-container">
              <p id="timestamp"></p>
            </div>
            <div id="attribution" class="attribution-container text-container-small">
  ...
  `;
    ...
    const cardTitle = this._config.card_title !== undefined 
    ...
    return html`
      <ha-card class="type-iframe">
        ...
        <div class="radar-container">
          <iframe class="radar-iframe" srcdoc=${doc} scrolling="no"></iframe>
        </div>
      </ha-card>
    `;
  }
